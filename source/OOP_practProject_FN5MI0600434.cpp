...
void playQuiz() {
    ...
    if (currentUser && currentUser->getRole() == Users::Role::PLAYER) {
...
int main() {
    ...
    users.push_back(new Admin("Admin", "User", "admin", "admin"));
    ...
    while (true) 
    {
        ...
        if (command == "signup") 
        {
            ...
            pendingQuizzes.push_back(q);  // чака одобрение
            ...
            if (!currentUser || currentUser->getRole() != Users::Role::ADMIN) {
            ...
            while (true) {
                ...
                if (subcommand == "view-users") {
                    for (size_t i = 0; i < users.size(); ++i) {
                        std::cout << i + 1 << ". " << users[i]->getUsername().c_str() << " ("
                            << (users[i]->getRole() == Users::Role::ADMIN ? "Admin" : "Player") << ")\n";
                    }
                }
                    ...
                    for (size_t i = 0; i < users.size(); ++i) {
                        if (users[i]->getUsername() == username) {
                            delete users[i];
                            // Manual erase:
                            for (size_t j = i; j < users.size() - 1; ++j) {
                                users[j] = users[j + 1];
                            }
                            users.pop_back();
                    ...
                    if (user && user->getRole() == Users::Role::PLAYER) {
